...
class NotaDbhelper {
  ...
  //método do tipo future (Async) vai retornar o Bando de dados
  Future<Database> _getDB() async{ //async
    return openDatabase(
      //colocar o endereço do DB
      join(await getDatabasesPath(), db_nome),
      onCreate: (db, version){ //se é a primeira vez que esta sendo executado ele cria o banco de dados
...
//create
  void create(Nota nota) async{
    try{
      ...
      await db.insert(table_nome, nota.toMap()); //insere os dados no banco
    } catch (e){
      print(e);
      return;
    }
...
//read
  Future<List<Nota>> getNotas() async{
    try{
      ...
      final List<Map<String,dynamic>> maps = await db.query(table_nome); //pega todos os dados dobanco
      //converter o MAP em List<Nota>
      return List.generate(maps.length, (e) => Nota.fromMap(maps[e])); //retorna a lista com o OBJ
    } catch(e){
      print(e); //printar erro
...
//Update
  void updateNota(Nota nota) async{
    try {
      ...
      //atualiza a notaa a partir do id
    } catch (e) {
      print(e);
      return;
    }
